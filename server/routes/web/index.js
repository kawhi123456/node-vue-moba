module.exports = app => {
    ...
    router.get('/news/init', async (req, res) => {
        ...
        const newsTitles = ["七城灯会，共贺四周岁！【点击回顾】千灯会精彩瞬间！", "王者模拟战玩法解读，策划Donny斗鱼直播首秀", "《王者荣耀》4岁生日快乐！", "王者千灯会节目时间表（线下游玩前必看）", "今日周年庆八大福利，梁祝上线，厨娘仙君归来~", "10月30日全服不停机更新公告", "【正式服】王者模拟战模式暂时关闭调优公告", "【已修复 陆续补发中】微信支付异常导致部分召唤师充值未到账说明", "10月29日版本不停机更新推迟说明", "【更新后修复】影忍之足唯一被动不生效异常说明", "感恩有你 李白新星元登场 峡谷全新福利来袭", "【周年许愿树】活动公告", "【周年庆典 明星抽内测】活动公告", "极致网速，快乐上分，中国电信邀你畅快赢好礼", "【回顾王者独家记忆 领专属局内称号】活动公告", "踏上新征途，第七届王者荣耀城市赛全国大赛开幕式暨小组赛即将开启！", "【KPL今日预报】VG vs AG超玩会，时隔512天梦泪再登KPL赛场", "你是赛评师：WE老板人皇Sky目送战队无缘季后赛，他们下赛季该如何调整？", "KGL王者荣耀职业发展联赛来袭，为梦而战该我上场", "高校区域联赛今日开启报名，海选赛首周号角同时吹响！"]
        const newsList = newsTitles.map(title => {
            const randowmCats = cats.slice(0).sort((a, b) => Math.random() - 0.5)
            return {
                categories: randowmCats.slice(0, 2),
                title: title
            }
        })
    ...
    router.get('/news/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([{
            ...
            },
            {
             ...
             // 用addFields来添加一个字段 此时newsList 已经存在  但是不需要所有的数据 只需要截取5条出来
            {
                $addFields: {
                    newsList: {
                        $slice: ['$newsList', 5]
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        // 聚合查询
        const cats = await Category.aggregate([
            // 第一步 用match操作符来指定一个条件查询 类似于sql中的where查询 可以将多个查询放在一个里面
            // 查parent字段等于parent中_id的
            { $match: { parent: parent._id } },
            // 第二步 关联查询 关联到heroes这个集合(表)  
            {
                $lookup: {
                    from: 'heroes',
                    localField: '_id', //本地字段
    ...
    router.get('/articles/:id',async(req, res) => {
        //lean()变成一个json对象 给data添加一个related自定义属性
